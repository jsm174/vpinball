...
bool RenderFrame::Execute(const bool log)
{
   ...
   if (!m_passes.empty())
   {
      ...
      for (auto pass : m_passes) // Always keep renderpasses rendering to an output window framebuffer, as well as explicitly forced passes
         if (lastPass != pass && (pass->m_rt->IsBackBuffer() || pass->m_forceExecution))
