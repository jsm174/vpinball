...
Video::Video(FlexDMD* pFlexDMD, const std::string& name)
...
Video* Video::Create(FlexDMD* pFlexDMD, AssetManager* pAssetManager, const std::string& path, const std::string& name, bool loop) {
   ...
   while (av_read_frame(pVideo->m_pFormatContext, pPacket) >= 0) {
    ...
    pVideo->SetLength(pVideo->m_frames.size() * pVideo->m_frameDuration);
    pVideo->Rewind();
    pVideo->Pack();
