    ...
    suspend fun loadTables(): List<Table> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                @Serializable data class TablesResponse(val success: Boolean, val tableCount: Int, val tables: List<Table>)

                val response = Json.decodeFromString<TablesResponse>(json)
                VPinballManager.log(VPinballLogLevel.INFO, "loadTables: Decoded ${response.tableCount} tables, success=${response.success}")
