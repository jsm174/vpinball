...
B2SSettings::B2SSettings()
{
   ...
   m_pluginsOn = false;
   m_pPluginHost = nullptr; // TODO: Remove PluginHost dependency

   ClearAll();

    // TODO: Load plugin settings from VPX plugin system
    m_pluginsOn = true; // Default to enabled for plugin version
}

B2SSettings::~B2SSettings()
{
   // Plugin system cleanup handled by VPX plugin framework
}
