...
void WebServer::EventHandler(struct mg_connection *c, int ev, void *ev_data)
{
   ...
   if (ev == MG_EV_HTTP_MSG) {
      ...
      if (message == "refresh_tables") {
         ...
         std::thread([c]() {
            VPINBALL_STATUS status = VPinballLib::VPinballLib::Instance().RefreshTables();

            std::string response = (status == VPINBALL_STATUS_SUCCESS)
            ...
            PLOGI.printf("WebServer: Table refresh completed with status: %s",
                        (status == VPINBALL_STATUS_SUCCESS) ? "success" : "error");
         }).detach();
      }
   }
...
void WebServer::Start()
{
   ...
   if (mg_http_listen(&m_mgr, bindUrl.c_str(), &WebServer::EventHandler, this)) {
      ...
      VPinballLib::VPinballLib::Instance().SendEvent(VPINBALL_EVENT_WEB_SERVER, &webServerData);
      ...
      m_pThread = new std::thread([this]() {
         ...
         VPinballLib::VPinballLib::Instance().SendEvent(VPINBALL_EVENT_WEB_SERVER, nullptr);
      });
   }
   else {
...
void WebServer::Delete(struct mg_connection *c, struct mg_http_message* hm)
{
   ...
   if (std::filesystem::is_regular_file(path)) {
      ...
      if (parentStr == prefStr) {
         // Use C++ API directly to get tables and check if this is a table folder
         const char* tablesJson = VPinballLib::VPinballLib::Instance().GetTables();
         if (tablesJson) {
            string jsonStr(tablesJson);
            // Simple check if UUID exists in the JSON
            if (jsonStr.find("\"" + dirName + "\"") != string::npos) {
               isTableFolder = true;
            }
         }
      }
      ...
      if (isTableFolder) {
         // Use C++ API directly to delete table
         VPINBALL_STATUS status = VPinballLib::VPinballLib::Instance().DeleteTable(dirName);
...
void WebServer::TableList(struct mg_connection *c, struct mg_http_message* hm)
{
   ...
   VPinballLib::VPinballLib::Instance().GetVPXTables(tablesData);
   ...
   VPinballLib::VPinballLib::Instance().FreeVPXTablesData(tablesData);
}
...
/*   string table;
   ...
   string exportPath = VPinballLib::VPinballLib::Instance().ExportTable(table);
   if (exportPath.empty()) {
...
{
   // Use C++ API directly to get tables JSON
   const char* tablesJson = VPinballLib::VPinballLib::Instance().GetTables();
   if (!tablesJson) {
      return tableId;
   }

   // Parse JSON to find table name
   string jsonStr(tablesJson);
   string searchKey = "\"uuid\":\"" + tableId + "\"";
   size_t pos = jsonStr.find(searchKey);
   if (pos != string::npos) {
      // Find the name field after the UUID
      size_t namePos = jsonStr.find("\"name\":\"", pos);
      if (namePos != string::npos) {
         namePos += 8; // Skip past "name":"
         size_t endPos = jsonStr.find("\"", namePos);
         if (endPos != string::npos) {
            return jsonStr.substr(namePos, endPos - namePos);
         }
      }
   }
