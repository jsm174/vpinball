...
void WebServer::EventHandler(struct mg_connection *c, int ev, void *ev_data)
{
   ...
   if (ev == MG_EV_HTTP_MSG) {
      ...
      string message(wm->data.buf, wm->data.len);
      ...
      if (message == "refresh_tables") {
         ...
         std::thread([c]() {
            ...
            string response = (status == VPINBALL_STATUS_SUCCESS)
...
void WebServer::Stop()
{
   ...
   // Send event immediately to update UI
   VPinballLib::VPinballLib::SendEvent(VPINBALL_EVENT_WEB_SERVER, nullptr);
}
...
void WebServer::TableList(struct mg_connection *c, struct mg_http_message* hm)
{
   ...
   VPinballLib::VPinballLib::Instance().GetTables(tablesData);
   ...
   VPinballLib::VPinballLib::Instance().FreeTablesData(tablesData);
}
