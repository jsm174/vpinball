...
JNIEXPORT jint JNICALL Java_org_vpinball_app_jni_VPinballJNI_VPinballLoadTable(JNIEnv* env, jobject obj, jstring source)
{
   const char* pSource = env->GetStringUTFChars(source, nullptr);
   VPINBALL_STATUS status = VPinballLoadTable(pSource);
...
JNIEXPORT jstring JNICALL Java_org_vpinball_app_jni_VPinballJNI_VPinballExportTable(JNIEnv* env, jobject obj, jstring uuid)
{
   ...
   return (exportPath != nullptr) ? env->NewStringUTF(exportPath) : nullptr;
}
...
JNIEXPORT jint JNICALL Java_org_vpinball_app_jni_VPinballJNI_VPinballSetTableImage(JNIEnv* env, jobject obj, jstring uuid, jstring imagePath)
{
   const char* pUuid = env->GetStringUTFChars(uuid, nullptr);
   const char* pImagePath = env->GetStringUTFChars(imagePath, nullptr);
   int result = VPinballSetTableImage(pUuid, pImagePath);
   env->ReleaseStringUTFChars(uuid, pUuid);
   env->ReleaseStringUTFChars(imagePath, pImagePath);
   return result;
}
...
JNIEXPORT jboolean JNICALL Java_org_vpinball_app_jni_VPinballJNI_VPinballDeleteFile(JNIEnv* env, jobject obj, jstring path)
{
   const char* pathChars = env->GetStringUTFChars(path, nullptr);
   bool deleted = VPinballDeleteFile(pathChars);
   env->ReleaseStringUTFChars(path, pathChars);
   return deleted;
}

JNIEXPORT jboolean JNICALL Java_org_vpinball_app_jni_VPinballJNI_VPinballCopyFile(JNIEnv* env, jobject obj, jstring sourcePath, jstring destPath)
{
   ...
   bool copied = VPinballCopyFile(sourceChars, destChars);
   env->ReleaseStringUTFChars(sourcePath, sourceChars);
   env->ReleaseStringUTFChars(destPath, destChars);
   return copied;
}

JNIEXPORT jstring JNICALL Java_org_vpinball_app_jni_VPinballJNI_VPinballStageFile(JNIEnv* env, jobject obj, jstring path)
{
   const char* pathChars = env->GetStringUTFChars(path, nullptr);
   const char* stagedPath = VPinballStageFile(pathChars);
   env->ReleaseStringUTFChars(path, pathChars);

   if (!stagedPath || stagedPath[0] == '\0')
      return nullptr;

   return env->NewStringUTF(stagedPath);
}
