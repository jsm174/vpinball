...
if [ "${OPENSSL_EXPECTED_SHA}" != "${OPENSSL_FOUND_SHA}" ]; then
   ...
   cd openssl
   ./Configure linux-aarch64 no-shared no-tests no-docs -fPIC --prefix="$(cd .. && pwd)/install" --openssldir="$(cd .. && pwd)/install"
   make -j${NUM_PROCS}
   make install_sw
   cd ..
...
SCORBIT_SDK_EXPECTED_SHA="${SCORBIT_SDK_SHA}-key1"
...
cp -a scorbit_sdk/scorbit_sdk/build/libscorbit_sdk.{so,so.*} ../../../third-party/runtime-libs/linux-aarch64
